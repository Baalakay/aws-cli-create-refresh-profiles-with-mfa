...
if [ ! -f "$CREDENTIALS_FILE" ] || ! grep -q "\[default\]" "$CREDENTIALS_FILE"; then
    ...
    echo "   Please run the setup script first: ./src/setup-aws-profiles.sh"
    exit 1
fi
...
if grep -A 2 "\[default\]" "$CREDENTIALS_FILE" | grep -q "aws_session_token"; then
    ...
    echo "   Please run the setup script to configure permanent credentials: ./src/setup-aws-profiles.sh"
    exit 1
fi
...
if ! grep -A 2 "\[default\]" "$CREDENTIALS_FILE" | grep -q "aws_access_key_id"; then
    ...
    echo "   Please run the setup script to configure credentials: ./src/setup-aws-profiles.sh"
    exit 1
fi
