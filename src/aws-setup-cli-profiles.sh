...
extract_existing_values() {
    ...
    if [ -f "$config_file" ]; then
        ...
        # Extract existing region
        existing_region=$(grep "region" "$config_file" | head -1 | cut -d'=' -f2 | xargs)
        
        # Extract existing output format
        existing_output=$(grep "output" "$config_file" | head -1 | cut -d'=' -f2 | xargs)
    fi
    
    echo "$existing_email|$existing_mfa_serial|$existing_region|$existing_output"
}
...
EXISTING_REGION=$(echo "$EXISTING_VALUES" | cut -d'|' -f3)
...
# Prompt for AWS Region with existing value option
if [ -n "$EXISTING_REGION" ]; then
    echo "Found existing AWS region: $EXISTING_REGION"
    echo ""
    echo "Options: y=keep, n=exit, r=replace"
    read -p "Keep existing AWS region? (y/n/r): " keep_region
    case $keep_region in
        [Yy]* )
            AWS_REGION="$EXISTING_REGION"
            echo "Using existing region: $AWS_REGION"
            ;;
        [Nn]* )
            echo "Exiting..."
            exit 0
        ...
        [Rr]* )
            while true; do
                read -p "Enter new AWS region (e.g., us-east-1, us-west-2, eu-west-1): " NEW_REGION
                if [ -z "$NEW_REGION" ]; then
                    echo "❌ Error: Region is required"
                    continue
                fi
                # Basic validation for region format
                if [[ ! "$NEW_REGION" =~ ^[a-z]+-[a-z]+-[0-9]+$ ]]; then
                    echo "❌ Error: Invalid region format. Use format like us-east-1, us-west-2, eu-west-1"
                    continue
                fi
                break
            done
            AWS_REGION="$NEW_REGION"
            echo "AWS region updated to: $AWS_REGION"
            # Save the new region back to the script file
            SCRIPT_FILE="$0"
            if [[ "$OSTYPE" == "darwin"* ]]; then
                sed -i '' "s/^AWS_REGION=.*/AWS_REGION=\"$AWS_REGION\"/" "$SCRIPT_FILE"
            else
                sed -i "s/^AWS_REGION=.*/AWS_REGION=\"$AWS_REGION\"/" "$SCRIPT_FILE"
            fi
            echo "✓ Region saved to script file for future use"
            ;;
        * )
    ...
    # If no existing region, prompt for it
    while true; do
        read -p "Enter AWS region (e.g., us-east-1, us-west-2, eu-west-1): " AWS_REGION
        if [ -z "$AWS_REGION" ]; then
            echo "❌ Error: Region is required"
            continue
        fi
        # Basic validation for region format
        if [[ ! "$AWS_REGION" =~ ^[a-z]+-[a-z]+-[0-9]+$ ]]; then
            echo "❌ Error: Invalid region format. Use format like us-east-1, us-west-2, eu-west-1"
            continue
        fi
        break
...
# Prompt for AWS Output Format with existing value option
if [ -n "$EXISTING_OUTPUT" ]; then
    echo "Found existing AWS output format: $EXISTING_OUTPUT"
    echo ""
    echo "Options: y=keep, n=exit, r=replace"
    read -p "Keep existing AWS output format? (y/n/r): " keep_output
    case $keep_output in
        [Yy]* )
            AWS_OUTPUT="$EXISTING_OUTPUT"
            echo "Using existing output format: $AWS_OUTPUT"
            ;;
        [Nn]* )
            echo "Exiting..."
            exit 0
        ...
        [Rr]* )
            while true; do
                read -p "Enter new AWS output format (json, text, table, yaml): " NEW_OUTPUT
                if [ -z "$NEW_OUTPUT" ]; then
                    echo "❌ Error: Output format is required"
                    continue
                fi
                # Validation for output format
                case $NEW_OUTPUT in
                    json|text|table|yaml)
                        break
                        ;;
                    *)
                        echo "❌ Error: Invalid output format. Use: json, text, table, or yaml"
                        continue
                        ;;
                esac
            done
            AWS_OUTPUT="$NEW_OUTPUT"
            echo "AWS output format updated to: $AWS_OUTPUT"
            # Save the new output format back to the script file
            SCRIPT_FILE="$0"
            if [[ "$OSTYPE" == "darwin"* ]]; then
                sed -i '' "s/^AWS_OUTPUT=.*/AWS_OUTPUT=\"$AWS_OUTPUT\"/" "$SCRIPT_FILE"
            else
                sed -i "s/^AWS_OUTPUT=.*/AWS_OUTPUT=\"$AWS_OUTPUT\"/" "$SCRIPT_FILE"
            fi
            echo "✓ Output format saved to script file for future use"
            ;;
        * )
            echo "Invalid option. Exiting..."
            exit 1
    ...
    # If no existing output format, prompt for it
    while true; do
        read -p "Enter AWS output format (json, text, table, yaml): " AWS_OUTPUT
...
echo "output = $AWS_OUTPUT" >> "$CONFIG_FILE"
echo "" >> "$CONFIG_FILE"
...
echo "- AWS output format: $AWS_OUTPUT"
...
echo "2. Run this script again" 
echo ""
echo "Now run the aws-refresh-mfa-token.sh script to set MFA session tokens for authentication"
echo "(re-run as needed when token expires - 12 hrs by default)"